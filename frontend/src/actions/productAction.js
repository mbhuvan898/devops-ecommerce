...
// THIS IS THE KEY LINE — USES .env OR FALLBACK
...
export const getProducts =
    (keyword = "", category = "", subcategory = "", brand = "", productType = "", price = [0, 200000], ratings = 0, currentPage = 1) => async (dispatch) => {
        try {
            ...
            let url = `${API_BASE}/api/v1/products?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}&page=${currentPage}`;
...
export const getSimilarProducts = (category) => async (dispatch) => {
    try {
        dispatch({ type: ALL_PRODUCTS_REQUEST });
        const { data } = await axios.get(`${API_BASE}/api/v1/products?category=${encodeURIComponent(category)}`);
...
export const deleteReview = (reviewId, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`${API_BASE}/api/v1/admin/reviews?id=${reviewId}&productId=${productId}`);
