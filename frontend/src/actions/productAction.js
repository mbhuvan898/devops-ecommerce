...
// Get Products By Category (Direct API call)
...
export const getProductsBySubcategory = (category, subcategory) => async (dispatch) => {
    try {
        dispatch({ type: ALL_PRODUCTS_REQUEST });

        const { data } = await axios.get(`/api/v1/products/category/${encodeURIComponent(category)}/subcategory/${encodeURIComponent(subcategory)}`);
