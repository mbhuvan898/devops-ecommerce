...
exports.getUserDetails = asyncErrorHandler(async (req, res, next) => {
  ...
  // Orders now live in order-service's own database, so ask over HTTP
  // instead of importing its model. If order-service is down we degrade
  // gracefully to 0 rather than failing the whole profile request.
  let ordersCount = 0;
  try {
    const orderServiceUrl = process.env.ORDER_SERVICE_URL || "http://order-service:4003";
    const { data } = await axios.get(
      `${orderServiceUrl}/internal/orders/count/${req.user.id}`,
      { timeout: 3000 }
    );
